...
struct RaceInProgressView: View {
    ...
    init(route: SavedRoute, locationManager: LocationManager, units: UnitPreference, reversed: Bool = false) {
        _vm = StateObject(wrappedValue: RaceViewModel(route: route, locationManager: locationManager, units: units, reversed: reversed))
    ...
    private var mapSection: some View {
        ZStack(alignment: .bottomTrailing) {
            RaceLiveMapViewBridge(
                ...
                roadCoords: vm.roadCoords,
                rallyCurves: vm.rallyCurves,
                routeTotalDistance: vm.routeTotalDistance,
                rallyMode: rallyMode
            )
            .ignoresSafeArea(edges: .top)

            // Re-center button — snap back to a pitched forward-facing camera
            Button {
                if let loc = vm.locationManager.currentLocation {
                    let course = loc.course >= 0 ? loc.course : 0
                    let speed  = max(loc.speed, 0)
                    let dist   = min(120 + speed * 10, 400)
                    let pitch  = min(55 + (1 - min(speed / 28, 1)) * 15, 70)
                    let cam    = MKMapCamera(
                    ...
                    raceMapViewInstance.setCamera(cam, animated: true)
                } else {
                ...
                if instr.direction == .finish {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Image(systemName: left ? "arrow.turn.up.left" : "arrow.turn.up.right")
                            .font(.system(size: 22, weight: .bold))
                            .foregroundStyle(color)
                        if let mod = instr.rallyModifier {
                            Text(mod.uppercased())
