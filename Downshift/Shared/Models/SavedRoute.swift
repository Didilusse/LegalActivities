...
struct SavedRoute: Identifiable, Codable, Hashable {
    ...
    var tags: [String] = []
    var location: String? = nil // Town/city name where the route is located
    ...
    init(id: UUID = UUID(), name: String, coordinates: [CLLocationCoordinate2D], createdDate: Date = Date(), raceHistory: [RaceResult] = [], difficulty: Difficulty = .medium, tags: [String] = [], location: String? = nil) {
        self.id = id
        self.name = name
    ...
    /// Returns a copy of this route with coordinates reversed (end becomes start)
    func reversedCopy() -> SavedRoute {
        SavedRoute(
            id: id,
            name: name,
            coordinates: clCoordinates.reversed(),
