...
class LocationManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private var pendingStartAfterAuth: Bool = false
    private var pendingForRace: Bool? = nil
    ...
    func requestLocation() {
        ...
        print("LocationManager.startTracking called. forRace=\(forRace)")
        if CLLocationManager.locationServicesEnabled() {
            print("Location services enabled = true, auth status = \(manager.authorizationStatus.rawValue)")
            let status = manager.authorizationStatus
            guard status == .authorizedWhenInUse || status == .authorizedAlways else {
                print("LocationManager: Cannot start tracking. Not authorized (status=\(status.rawValue)). Requesting... forRace=\(forRace)")
                if status == .notDetermined {
                    pendingStartAfterAuth = true
                    pendingForRace = forRace
                    print("LocationManager: Deferring start until auth. pendingStartAfterAuth=\(pendingStartAfterAuth), pendingForRace=\(String(describing: pendingForRace))")
                    manager.requestWhenInUseAuthorization()
                }
                return
            }
            print("LocationManager: Authorized immediately. Proceeding to beginLocationUpdates(forRace: \(forRace))")
            beginLocationUpdates(forRace: forRace)
        } else {
            print("Location services are not enabled.")
        }
    }
    
    private func beginLocationUpdates(forRace: Bool) {
        print("beginLocationUpdates invoked at \(Date())")
    ...
    func locationManagerDidChangeAuthorization(_ manager: CLLocationManager) {
        print("locationManagerDidChangeAuthorization called with status=\(manager.authorizationStatus.rawValue)")
        let status = manager.authorizationStatus
        if status == .authorizedWhenInUse || status == .authorizedAlways {
            print("Location permission granted.")
            if pendingStartAfterAuth {
                print("Auth granted. pendingStartAfterAuth was true. pendingForRace=\(String(describing: pendingForRace))")
                let mode = pendingForRace ?? false
                pendingStartAfterAuth = false
                pendingForRace = nil
                beginLocationUpdates(forRace: mode)
            }
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        print("didUpdateLocations called. count=\(locations.count). First call after start? prev=\(previousLocation == nil)")
        ...
        guard latestRaw.horizontalAccuracy >= 0 && latestRaw.horizontalAccuracy < 65 else { // Max 65m accuracy threshold
            print("LM Update: Poor accuracy: \(latestRaw.horizontalAccuracy)m at \(latestRaw.coordinate.latitude),\(latestRaw.coordinate.longitude) t=\(latestRaw.timestamp). Update ignored for distance.")
